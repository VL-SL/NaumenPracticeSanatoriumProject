...
const CabinetsModule = () => {
      ...
      try {
        ...
        const medicalStaff = usersData.filter((user) => {
          ...
          return user.roles.some(
            (role) => role === "ROLE_DOCTOR" || role === "ROLE_NURSE",
          );
        });
        ...
        const assignmentsResponse =
          await AdminDashboardService.getAllAssignments(token);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let cabinetId;
      if (isEditing) {
        const updatedCabinet = await AdminDashboardService.updateCabinet(
          selectedCabinet.id,
          formData,
          token,
        );
        ...
        const newCabinet = await AdminDashboardService.createCabinet(
          formData,
          token,
        );
        ...
        try {
          await Promise.all(
            formData.assignedUsers.map((user) =>
              AdminDashboardService.assignCabinetToStaff(
                user.id,
                cabinetId,
                token,
              ),
      ...
      const message =
        error.response?.data?.message || "Ошибка сохранения кабинета";
    ...
    if (window.confirm("Вы уверены, что хотите удалить этот кабинет?")) {
      try {
        ...
        const updatedCabinets =
          await AdminDashboardService.getAllCabinets(token);
        setCabinets(updatedCabinets);
      } catch (error) {
  ...
  const handleAssignUser = async (cabinetId, userId) => {
    try {
      await AdminDashboardService.assignCabinetToStaff(
        userId,
        cabinetId,
        token,
      );
  ...
  const handleRemoveAssignment = async (cabinetId, userId) => {
    try {
      await AdminDashboardService.removeCabinetFromStaff(
        userId,
        cabinetId,
        token,
      );
      ...
      if (updatedAssignments[cabinetId]) {
        updatedAssignments[cabinetId] = updatedAssignments[cabinetId].filter(
          (u) => u.id !== userId,
        );
                          ...
                          {currentAssignments[cabinet.id].map((user) => (
                            <div key={user.id} className="assigned-user-item">
                              ...
                              <button
                                ...
                                onClick={() =>
                                  handleRemoveAssignment(cabinet.id, user.id)
                                }
                              >
