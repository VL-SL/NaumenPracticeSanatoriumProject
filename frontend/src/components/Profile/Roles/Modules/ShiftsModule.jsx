...
const ShiftsModule = () => {
      ...
      const filtered = shifts.filter(
        (shift) =>
          ...
          (shift.description &&
            shift.description
              .toLowerCase()
              .includes(shiftSearchTerm.toLowerCase())),
      );
  ...
  const handleShiftSubmit = async (e) => {
    ...
    try {
      if (isEditingShift) {
        await AdminDashboardService.updateShift(
          selectedShift.id,
          shiftFormData,
          token,
        );
      ...
      const message =
        error.response?.data?.message || "Ошибка сохранения смены";
  ...
  const handleToggleShiftStatus = async (shiftId, isActive) => {
    try {
      ...
      showNotification(
        `Смена ${!isActive ? "активирована" : "деактивирована"}`,
        "success",
      );
      const updatedShifts = await AdminDashboardService.getAllShifts(token);
      setShifts(updatedShifts);
          ...
          <form onSubmit={handleShiftSubmit}>
            <div className="form-row">
              <div className="form-group">
                ...
                />
                {shiftErrors.name && (
                  <div className="error-message">{shiftErrors.name}</div>
                )}
              </div>
            </div>
            <div className="form-row">
              <div className="form-group">
                ...
                <input
                  ...
                  required
                />
                {shiftErrors.startDate && (
                  <div className="error-message">{shiftErrors.startDate}</div>
                )}
              </div>
              <div className="form-group">
                ...
                />
                {shiftErrors.endDate && (
                  <div className="error-message">{shiftErrors.endDate}</div>
                )}
              </div>
            </div>
            <div className="form-row">
              <div className="form-group">
                ...
                <textarea
                  ...
                  maxLength="200"
                />
                {shiftErrors.description && (
                  <div className="error-message">{shiftErrors.description}</div>
                )}
              </div>
            </div>
            <div className="form-row">
      ...
      <div className="users-table-container">
            ...
            {currentShifts.length > 0 ? (
              currentShifts.map((shift) => (
                <tr key={shift.id}>
                  ...
                  <td>
                    <div
                      className={`admin-status-badge ${shift.isActive ? "active" : "inactive"}`}
                    >
                      {shift.isActive ? "Активна" : "Неактивна"}
                      <button
                        className="admin-toggle-status-btn"
                        onClick={() =>
                          handleToggleShiftStatus(shift.id, shift.isActive)
                        }
                      >
                        <i
                          className={`fas ${shift.isActive ? "fa-toggle-on" : "fa-toggle-off"}`}
                        ></i>
                      </button>
                    </div>
                  </td>
        ...
        {filteredShifts.length > shiftsPerPage && (
          <div className="pagination">
            ...
            {Array.from({ length: totalShiftPages }, (_, i) => i + 1).map(
              (number) => (
                <button
                  key={number}
