    ...
    const fetchShifts = async () => {
      try {
        ...
        if (activeResponse.data.length > 0) {
          ...
          const activeShiftIndex = allResponse.data.findIndex(
            (shift) => shift.id === firstActiveShiftId,
          );
          if (activeShiftIndex >= 0) {
            const pageWithActiveShift = Math.ceil(
              (activeShiftIndex + 1) / shiftsPerPage,
            );
    ...
    <div className="sp-shifts-page-container">
      ...
      <div className="sp-shifts-content-wrapper">
        {currentShifts.length > 0 ? (
          ...
          <div className="sp-no-shifts-message">Нет смен для отображения</div>
        )}
      </div>
    </div>
  );
